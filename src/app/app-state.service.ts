...
export class AppStateService {
  ...
  filteredTasks = computed(() => {
    ...
    if (view === 'MyTasks') {
      ...
      return filtered; // Show everything for 'All Tasks'
    } else if (view === 'Closed') {
  ...
  pieChartGradient = computed(() => {
    ...
    // Segments: Claimed (#003366), Submitted (#fbbc04), Closed (#4caf50), Overdue (#ddd)
    return `conic-gradient(
      #003366 0% ${pClaimed}%, 
      #fbbc04 ${pClaimed}% ${pClaimed + pSubmitted}%,
      #4caf50 ${pClaimed + pSubmitted}% ${pClaimed + pSubmitted + pClosed}%, 
      #ddd ${pClaimed + pSubmitted + pClosed}% ${pClaimed + pSubmitted + pClosed + pOverdue}%,
      #f1f3f4 ${pClaimed + pSubmitted + pClosed + pOverdue}% 100%
    )`;
  });
  ...
  claimTask() {
    const task = this.selectedTask();
    if (task) {
      this.taskService.updateTask(task.id, { isClaimed: true, claimedBy: 'Current User' });
      this.selectedTask.set({ ...task, isClaimed: true, claimedBy: 'Current User' });
    }
  }
